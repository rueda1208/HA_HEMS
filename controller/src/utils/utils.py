...
def get_target_temperature(zone_id: str, gdp_events: List[Dict[str, Any]], hvac_mode: str | None = None) -> Union[float, None]:
    ...
    if not gdp_events:
        ...
        if current_hour in gdp_hours_list:
            logger.debug("Current hour is within a GDP event, applying flexibility adjustment")
            flexibility = zone_flexibility.get("downward", 0.0)
        else:
            flexibility = 0.0
        ...
        for slot_range, slot_data in time_slots.items():
            ...
            if start_hour < end_hour:
                if start_hour <= current_hour < end_hour:
                    return slot_data.get("target_temp_C") - flexibility
            else:
